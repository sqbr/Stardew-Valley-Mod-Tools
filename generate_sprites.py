    ...
    if L < 5:
        ...
        if D == 0:
           return newC 
            ...
            X = L/D  #ratio
            return (int(X*newC[0]), int(X*newC[1]), int(X*newC[2]))     
...
def create_code(folder, sprite_name):
    ...
    if in_dictionary(files_translated, folder,sprite_name) and mode =="UI": #uses different versions depending on language
        code+="         \"FromFile\": \"assets/{{Target}}_{{language}}.png\",\n"
    else:
        code+="         \"FromFile\": \"assets/{{Target}}.png\",\n"
    if in_dictionary(files_onIsland, folder,sprite_name):
             code+="         \"When\":{\n"
             code+="             \"LocationContext\": \"Valley\",\"LocationName | contains=Desert\": \"false\"\n"
             code+="         },\n"   
             code+="         \"Update\": \"OnLocationChange\",\n" 
    code+="         \"PatchMode\": \"Overlay\"\n"             
    code+="      },\n\n"  
    return code
    ...
    for sprite_name in items:
        if not in_dictionary(files_not[mode],folder,sprite_name):
            ...
            if len(split_name)==2: #not translated, translated versions processed separately
                ...
                if in_dictionary(files_translated, folder,sprite_name) and mode == "UI": #has translations
                    split_name = sprite_name.split(".")
                    short_name = split_name[0]
...
def process_image(folder,sprite_name):
    ...
    if len(split_name)==2: #not translated
        if in_dictionary(files_translated, folder,sprite_name) and mode =="UI": #add a file for english
...
def process_items_recolour(items,folder,content):
    #process every element of the list items, which is contained within folder, write code to content
    global mode
    for sprite_name in items:
        if not in_dictionary(files_not[mode],folder,sprite_name):
...
def process_image_recolour(folder,sprite_name):
    global mode, have_image
    image_string= end_path[mode] +"assets/"+ folder + "/"+ sprite_name   
    print(folder+"/"+sprite_name)
    img = Image.open(image_string)      
    save_string = end_path["recolour"] + "assets/"+folder + "/"+ sprite_name  
...
def process_folder_recolour():
    global mode, have_image
    content = open(end_path["recolour"]+"content.json","w")
    content.write("{\n")
    content.write("   \"Format\": \"1.27.0\",\n")
    content.write("   \"Changes\": [\n")
    for mode in ["ground","wood","plants","fall","flowers"]:
        for folder in folders[mode]: #simple folder
            print("1")
            items =list_directory(start_path+folder+'/',"*.png")  #all images in plants folder
            process_items_recolour(items,folder,content)             
        for folder in folders2[mode]: #nested folder
            print("2")
            print("folder "+folder)
            items =list_directory(start_path+folder+'/',"*.png")  #blah.thing.png
...
#process_image("TerrainFeatures","hoeDirt.png")
#process_image(".","basic_rainbow.png")
#mode = "flowers"
#process_rainbow()

process_folder_recolour()
